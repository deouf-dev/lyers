...
module.exports = {
    ...
    async run(client, member){
        ...
        const {toggle, punish, limit} = guildData.get("antiraid").antimassban;
        ...
        if(toggle === "on" && client.util.isWhitelist(guild, message.author.id) || toggle === "max" && client.util.isOwner(guild, executor.id)) return client.util.antiraidLog(guild, `${executor} a ban ${member}`)
        ...
        if(limit.split("/")[0] <= data){
            const punished = await client.util.punish(punish, executorMember, "Antimassban - lYers 📘");
            client.util.antiraidLog(guild, `${executor} a banni plus de ${limit.split("/")[0]} personne(s)${punished.punished ? `Je l'ai` : "Je n'ai pa pu le"} ${punish}`)
